import { escapeHtml, formatDate, icon, progressBar, ui } from "../ui.js";
...
function deckCard(deck, index = 0) {
  ...
  const disabled = deck.action === "Mastered";
  ...
  const deckHref = `/app/sentence-mining/decks/${encodeURIComponent(deck.id)}`;
  ...
  return `
    <article class="group relative w-[254px] shrink-0 overflow-hidden rounded-lg border border-brand-line/80 bg-brand-panel shadow-[0_1px_2px_rgba(29,41,63,.06)] transition duration-200 hover:-translate-y-1 hover:scale-[1.015] hover:border-brand-orange/45 hover:shadow-[0_22px_42px_rgba(29,41,63,.13)] sm:w-[282px]">
      <a class="block text-inherit no-underline" href="${deckHref}" data-deck-link aria-label="Open ${escapeHtml(deck.name || deck.title)} deck">
      ...
      <div class="grid gap-3 p-4 pt-3">
        <div class="grid grid-cols-[1fr_auto_auto] gap-2">
          <a class="${disabled ? "pointer-events-none inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-brand-line/80 bg-white/50 px-3 py-2 text-sm font-bold text-brand-graphite no-underline" : "inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-brand-ink px-3 py-2 text-sm font-bold text-white no-underline transition hover:bg-brand-redDark"}" href="/app/review?deckId=${encodeURIComponent(deck.id)}" data-app-link aria-disabled="${disabled ? "true" : "false"}">
            ${icon(deck.action === "Practice Review" ? "book" : deck.action === "Start" ? "play" : "check", "h-4 w-4")}
            <span>${escapeHtml(deck.action)}</span>
          </a>
  ...
  return `
    <div>
      ...
      <form class="mt-5 grid gap-3" data-form="sentenceDeck">
        ...
        <label class="${ui.label}">Deck image URL<input class="${ui.input}" name="imageUrl" maxlength="1000" placeholder="https://..."></label>
  ...
  return `
    <div>
      ...
      <form class="mt-5 grid gap-3" data-form="deckSentence">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="${ui.label}">Audio URL<input class="${ui.input}" name="audioUrl" maxlength="1000" placeholder="https://..."></label>
          <label class="${ui.label}">Image URL<input class="${ui.input}" name="imageUrl" maxlength="1000" placeholder="https://..."></label>
        </div>
        <div class="grid gap-3 sm:grid-cols-2">
...
export function sentenceMiningView({ state }) {
  const decks = state.sentenceDecks || [];
  ...
  return `
    <article class="overflow-hidden rounded-lg border border-brand-line/80 bg-brand-panel shadow-[0_1px_2px_rgba(29,41,63,.06)] transition hover:-translate-y-1 hover:border-brand-orange/45 hover:shadow-[0_18px_36px_rgba(29,41,63,.12)]">
      <div class="bg-gradient-to-br from-brand-blue via-brand-teal to-brand-panel p-4 text-white">
        ...
        ${topic.description ? `<p class="mt-2 line-clamp-2 text-sm leading-5 text-white/80">${escapeHtml(topic.description)}</p>` : ""}
      </div>
      <div class="grid gap-3 p-4">
        ...
        <p class="line-clamp-3 min-h-16 text-sm leading-6 text-brand-charcoal">${escapeHtml(topic.sentences?.[0]?.translation || "Open this topic in review to practice the sentences inside.")}</p>
        ${deck.owner && !deck.system ? `<div class="grid grid-cols-2 gap-2"><button class="${ui.secondary}" data-action="openEditTopicModal:${topic.id}">${icon("edit")}<span>Edit</span></button><button class="${ui.danger}" data-action="openDeleteTopicModal:${topic.id}">${icon("trash")}<span>Delete</span></button></div>` : ""}
      </div>
    </article>
  `;
}
    ...
    <div class="-m-4 min-h-[calc(100vh-80px)] bg-brand-cream px-4 py-5 text-brand-ink sm:-m-6 sm:px-6 lg:-m-7 lg:px-7">
      <section class="overflow-hidden rounded-lg border border-brand-line/80 bg-brand-panel shadow-[0_18px_42px_rgba(29,41,63,.08)]">
        <div class="relative min-h-44 bg-gradient-to-br from-brand-red via-brand-amber to-brand-panel p-5 sm:p-7">
          ...
          <div class="flex flex-wrap gap-2">
            <a class="${ui.primary}" href="/app/review?deckId=${encodeURIComponent(deck.id)}" data-app-link>${icon("book")}<span>${escapeHtml(deck.action === "Start" ? "Start" : deck.action)}</span></a>
      ...
      <div class="mt-6 grid gap-4">
        ${(deck.topics || []).length ? `<section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">${deck.topics.map((topic) => topicSection(topic, deck)).join("")}</section>` : sentenceDeckTable(deck)}
      </div>
    </div>
  `;
}
